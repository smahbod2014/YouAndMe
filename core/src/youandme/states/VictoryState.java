...
public class VictoryState extends State {
	...
	public VictoryState(GSM gsm, State previous, Entity entity) {
		super(gsm);
		...
		buttons.add(new TextButton(YouAndMe.WIDTH/2, YouAndMe.HEIGHT*3/4 - 40, 3, "Nice job", -1, 0));
		buttons.add(new TextButton(YouAndMe.WIDTH/2, YouAndMe.HEIGHT/2 - 40, 2, "Next level", C.BUTTON_NEXT_LEVEL, 0));
		buttons.add(new TextButton(YouAndMe.WIDTH/2, YouAndMe.HEIGHT/2-90, 2, "Main menu", C.BUTTON_MAIN_MENU, 0));
		...
		if (Gdx.input.justTouched()) {
			...
			for (TextButton tb : buttons) {
				if (tb.contains(mouse.x, mouse.y)) {
					if (tb.id == C.BUTTON_NEXT_LEVEL) {
						gsm.set(new PlayState(gsm, LevelReader.level + 1));
